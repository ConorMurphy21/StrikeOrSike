...
export const useGameStore = defineStore("game", {
      ...
      room.players.forEach((player) => {
        if (player.active && player.id !== self) {
          const match = this.matches.find(
            (match: Match) => match.player.id === player.id,
          );
  ...
  actions: {
    ...
    unuseResponse(response: string) {
      const room = useRoomStore();
      this.responses[room.self!.id].used = this.responses[
        room.self!.id
      ].used.filter((r) => r !== response);
    },
    async startTimer() {
      if (this.timeoutId) {
    ...
    matchesFound(matches: ServerMatch[]) {
      ...
      for (const match of matches) {
        const player = room.players.find(
          (player: Player) => player.id === match.player,
        );
        if (!player) continue;
        const stateMatch = this.matches.find(
          (sm: Match) => sm.player.id === player.id,
        );
        if (stateMatch) {
    ...
    unmatch() {
      const room = useRoomStore();
      const selfId = room.self!.id;
      const usedResponse = this.matches.find(
        (match: Match) => match.player.id === selfId,
      )!.response;
    ...
    async getResponses(id: string): Promise<void> {
      return new Promise((resolve, reject) => {
        ...
        socket.emit(
          "getResponses",
          id,
          (data: { success: boolean; responses: Responses }) => {
            if (data.success) {
    ...
    bindEvents() {
      ...
      socket.on(
        "nextSelection",
        (data: { selector: string; selectionType: string }) => {
          const room = useRoomStore();
          const selector = room.players.find(
            (player: Player) => player.id === data.selector,
          );
          // before we clear matches, make sure we used our match, this can happen if a next selection happens between
          // unmatch and match
          const selfId = room.self!.id;
          const selfMatch = this.matches.find(
            (match: Match) => match.player.id === selfId,
          );
          if (
            selfMatch &&
            !this.responses[selfId].used.includes(selfMatch.response)
          )
      ...
      socket.on("midgameConnect", (data: MidgameConnectData) => {
        ...
        this.selector = room.players.find(
          (player) => player.id === data.selector,
        );
