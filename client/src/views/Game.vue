<template>
  <div
    class="main-content d-flex w-xl-75 w-100 flex-grow-1 p-1 position-relative">
    <a
    ...
    <div
      v-if="!isMobile"
      class="position-fixed bottom-0 end-0 m-4 gap-1 d-flex justify-content-end align-items-end">
...
export default defineComponent({
  ...
  computed: {
    ...
    helpLink() {
      ...
      return this.$router.resolve({ name: "howToPlay", hash: tips[this.scene] })
        .href;
    },
    isMobile() {
  ...
  mounted() {
    if (this.roomName !== this.storeRoomName) {
      void this.$router.push({ name: "home", query: { name: this.roomName } });
    }
  },
});
</script>
